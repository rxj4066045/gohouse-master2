...
public class PermissionUtil {
    ...
    private static void permissions(Activity context, PermissionsInterface permissionsInterface, boolean isShowDialog, String[] permissions) {
        permissions(context, permissionsInterface, true, isShowDialog, permissions);
    }

    private static void permissions(final Activity context, final PermissionsInterface permissionsInterface, boolean isoneRefuse1, final boolean isShowDialog, String[] permissions) {
        ...
        new RxPermissions(context).requestEach(ss).subscribe(new Consumer<Permission>() {
            ...
            public void accept(Permission permission) throws Exception {
                ...
                if (permission.granted) {
                    ...
                    if (isShowDialog)
                        showRefuseDialog(context, permissionsInterface, permission, ss);
                    ...
                    if (isShowDialog)
                        showWithNever(context, permission);
                }
    ...
    private static void showRefuseDialog(final Activity context, final PermissionsInterface permissionsInterface, final Permission permission, final String[] Permissionsss) {

        if (refuseDialog != null && refuseDialog.isShowing()) {
            return;
        }
        String msg = "你拒绝了" + PermissionInit.getPermissionName(permission.name) + "等权限," + getAppName(context) + "或无法正常运行，是否重新申请该权限?";
        ...
        builder.setTitle("提示")
                ...
                .setPositiveButton("重新申请", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        permissions(context, permissionsInterface, false, Permissionsss);
                    }
                })
    ...
    private static void showWithNever(final Activity context, Permission permission) {
        if (withNeverDialog != null && withNeverDialog.isShowing()) {
            return;
        }
        String msg = "由于" + getAppName(context) + "无法获取到" + PermissionInit.getPermissionName(permission.name) + "等权限,无法正常运行,请开户权限后再使用。\n" +
    ...
    public static class Builder {
        ...
        public void execute(PermissionsInterface permissionsInterface) {
            ...
            PermissionUtil.permissions(context, permissionsInterface, isShowDialog, ps);
        }
