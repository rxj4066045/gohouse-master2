package com.picturepicker.picturepicker.utils;
 ...
 * <p>
 * 裁剪：
 * PhotoUtils.cropImage(activity,imageFile,authority).……设置剪裁参数…….crop(获取回调);
 * <p>
 * 调用相机：
 * PhotoUtils.camera(activity,authority).toCamera(回调数据);
 *
 * 选择单张图片
 * PhotoUtils.selectPhoto(activity).selectPhoto();
 *
 * 搜索所有图片
 * PhotoUtils.getPictures(activity).achieve()
 */
...
public class PhotoUtils {
    ...
    //从相册中选择
    private static void selectPhoto2(FragmentActivity activity, SelectPhotoBack selectPhotoBack){
        getSelectAndCropFragment(activity).selectPhoto(selectPhotoBack);
    }

    //返回所有图片路径
    private static void getPictures(FragmentActivity activity, Fragment fragment, PicturesCallBack callBack){
        if(fragment != null){
            getSelectAndCropFragment(fragment).getPictures(callBack);
        }else if(activity != null)
        getSelectAndCropFragment(activity).getPictures(callBack);
    }
    ...
    public static CropImage cropImage(@NonNull Activity activity, @NonNull String imagePath, @NonNull String authority) {
        return new CropImage(activity, authority, new File(imagePath));
    }
     ...
     * @return
     */
    public static SelectPhoto selectPhoto(@NonNull Activity activity){
        return new SelectPhoto(activity);
    }


    public static GetPictures getPictures(@NonNull Activity activity){
        return new GetPictures(activity);
    }
    public static GetPictures getPictures(@NonNull Fragment fragment){
        return new GetPictures(fragment);
    }

    public static class SelectPhoto{
        FragmentActivity activity;
        private SelectPhoto(@NonNull Activity activity){
        ...
        public void selectPhoto(SelectPhotoBack selectPhotoBack){
            selectPhoto2(activity,selectPhotoBack);
        }
    ...
    public static class GetPictures{
        FragmentActivity activity;
        Fragment fragment;
        private GetPictures(@NonNull Activity activity){
            if (!(activity instanceof FragmentActivity)) {
                throw new RuntimeException("activity 需要FragmentActivity类型");
            } else {
                this.activity = (FragmentActivity) activity;
        ...
        private GetPictures(@NonNull Fragment fragment){
            this.fragment = fragment;
        }

        public void achieve(PicturesCallBack callBack){
            getPictures(activity,fragment,callBack);
        }
    ...
    public static class CropImage {
        ...
        public void crop(CropImageBack cropImageBack) {
    ...
    public interface CameraImageBack {
        void cameraImage(File file);
        void erron(String err);
    }

    //选择图片
    public interface SelectPhotoBack {
    ...
    public interface PicturesCallBack {
        void callBack(Map<String, List<String>> pictures);
    }
    ...
    private static SelectAndCropFragment getSelectAndCropFragment(Fragment fragment) {
        String tab = "Crop";
        FragmentManager fragmentManager = fragment.getChildFragmentManager();
        Fragment childfragment = null;
        if (childfragment == null) {
            childfragment = new SelectAndCropFragment();
            fragmentManager.beginTransaction().add(childfragment, tab).commitAllowingStateLoss();
            fragmentManager.executePendingTransactions();
        }

        return (SelectAndCropFragment) childfragment;
    }
    ...
    private static class Utils {
        ...
        private static class ImageConvertUtils {
            ...
            private static File bitmapToFile(@NonNull Context context, @NonNull Bitmap bitmap) {
                String cacheFileNmame = System.currentTimeMillis() + (int) (Math.random() * 1000) + ".png";
...
//                //如果是文件。Uri.fromFile(File file)生成的uri。那么下面这个方法可以得到结果
    ...
    public static class SelectAndCropFragment extends Fragment {
        ...
        private static final int requestCodeselectPhoto = 10003;
        private String cameraImageName = null;
        private CameraImageBack cameraImageBack;
        private SelectPhotoBack selectPhotoBack;
        private PicturesCallBack pictureCallBack;
        ...
        //选择图片
        private void selectPhoto(SelectPhotoBack selectPhotoBack){
            this.selectPhotoBack = selectPhotoBack;
        ...
        //选择多张图片
        private void getPictures(PicturesCallBack callBack){
            pictureCallBack = callBack;
            new GetPicturesAsyncTask().execute();
        }


        private class GetPicturesAsyncTask extends AsyncTask<Void,Void,Map<String,List<String>>>{
            @Override
            protected Map<String, List<String>> doInBackground(Void... voids) {
                ...
                Map<String,List<String>> photos = new LinkedHashMap<>();

                while (mCursor.moveToNext()) {
                    //获取图片的路径
                    String path = mCursor.getString(mCursor
                            .getColumnIndex(MediaStore.Images.Media.DATA));
                    if(Utils.SDCardUtils.isFileExists(path)){
                        //获取该图片的父路径名
                        String parentName = new File(path).getParentFile().getName();
                        List<String> list = photos.get(parentName);
                        if(list == null){
                            list = new ArrayList<>();
                        }
                        list.add(path);
                        photos.put(parentName,list);
                    }
            ...
            @Override
            protected void onPostExecute(Map<String, List<String>> map) {
                super.onPostExecute(map);
                if(pictureCallBack != null){
                    pictureCallBack.callBack(map);
                }
                signOut();
            }
        }
        ...
        //裁剪图片
        private void cropImage(@NonNull File file, String authority, int aspectX, int aspectY, int outputX, int outputY, CropImageBack cropImageBack) {
        ...
        public void onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            if (requestCode == requestCode_crop) {
                ...
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if (cameraImageBack != null) {
                        if (file != null)
                            cameraImageBack.cameraImage(file);
                        else
                            cameraImageBack.erron("获取照片失败");
                    }
                } else if (cameraImageBack != null)
                    cameraImageBack.erron("获取照片失败");
            } else if(requestCode == requestCodeselectPhoto){
                if (resultCode == Activity.RESULT_OK){
                    Uri uri = data.getData();
                    String path = Utils.ImageConvertUtils.uriToPath(getContext(), uri);
                    if(selectPhotoBack != null)
                        selectPhotoBack.selectPhoto(path);
                }else {
                    if(selectPhotoBack != null)
                        selectPhotoBack.erron("获取图片失败");
                }
            }
